...
const main = async () => {
  ...
  console.log(`Alice free balance in readable format: ${prettyBalance(aliceAccount.data.free)}`);
  ...
  const unsubAlice = await api.query.system.account(alice.address, aliceAcct => {
    ...
    console.log(`Alice Account (sub): ${prettyBalance(aliceFreeSub)}`);
  });
  const unsubBob = await api.query.system.account(bob.address, bobAcct => {
    ...
    console.log(`Bob Account (sub): ${prettyBalance(bobFreeSub)}`);
  });
